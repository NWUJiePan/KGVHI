...
if __name__ == '__main__':
    ...
    relationship_matrix.to_csv('Human_VIRUS_interaction_Matrix.csv')

    store_csv(negative_sample, 'NegativeSample_all_HB.csv')
    negative_sample_train = negative_sample[0: int(0.7 * len(negative_sample))]
    store_csv(negative_sample_train, 'NegativeSample_Train_all_HV.csv')
    negative_sample_validation = negative_sample[int(0.7 * len(negative_sample)):int(0.8 * len(negative_sample))]
    store_csv(negative_sample_validation, 'NegativeSample_Validation_all_HV.csv')
    negative_sample_test = negative_sample[int(0.8 * len(negative_sample)):]
    store_csv(negative_sample_test, 'NegativeSample_Test_all_HV.csv')
